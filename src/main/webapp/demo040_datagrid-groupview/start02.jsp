...
$(function() {
	...
	$('#tt').datagrid({
		...
		columns:[[
			{field:'lendName',title:'第三资方账户',width:100,sortable:true},
			{field:'total',title:'日限额',width:80,align:'right',sortable:true},
			{field:'userd',title:'已放款金额',width:80,align:'right',sortable:true},
			{field:'remain',title:'剩余金额',width:80,align:'right',sortable:true},
			{field:'count',title:'笔数',width:80,align:'right',sortable:true},
			{field:'oneTimeTotal',title:'单笔最大限额',align:'right',fitColumns:true,sortable:true},
			{field:'classifyid',title:'放款品类',width:500,align:'left',sortable:true},
			{field:'type',title:'种类',width:80,align:'right',sortable:true,formatter:function formatType(type) {
				 if(type==1){
					 return "第三资方";
				 }else if(type==2){
					 return "债转渠道";
		...
		groupFormatter:function(value, rows){
			...
			return formatType(rows[0].type)+"：总共："+total+"，已使用："+userd+"，剩余："+remain;
		}
	});
